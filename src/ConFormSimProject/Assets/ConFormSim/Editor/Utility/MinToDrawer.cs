    ...
    public class MinToDrawer : PropertyDrawer
    {
                                ...
                                GUIContent label)
        {
            ...
            else if (type == SerializedPropertyType.Integer)
            {
                ...
                {
                    EditorGUI.HelpBox(ctrlRect, "Min int not found!!", MessageType.Info);
                    return;
                }
                float minVal = minProperty.intValue;
                float maxVal = property.intValue;
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    minProperty.intValue = (int) minVal;
                    property.intValue = (int) maxVal;
                }
            }
            else
                EditorGUI.HelpBox(ctrlRect, "MinTo is for Vector2 or float or int!!", MessageType.Error);
        }
